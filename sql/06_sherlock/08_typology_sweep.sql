...
CREATE OR REPLACE VIEW investec_fraud_aml_gold.typology_exposure AS
WITH gaming AS (
  ...
  FROM investec_fraud_aml_silver.transactions t
  JOIN investec_fraud_aml_silver.accounts a USING (account_id)
...
FROM gaming g
JOIN investec_fraud_aml_silver.customers c ON c.customer_id = g.customer_id
