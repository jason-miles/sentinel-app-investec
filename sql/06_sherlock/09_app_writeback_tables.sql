-- Investec Sentinel — app write-back tables (create-if-not-exists).
--
...
CREATE TABLE IF NOT EXISTS investec_fraud_aml_gold.sherlock_case_notes (
  note_id    STRING,
  case_id    STRING,
  author     STRING,
  note       STRING,
...
CREATE TABLE IF NOT EXISTS investec_fraud_aml_gold.sherlock_case_actions (
  action_id  STRING,
  case_id    STRING,
...
CREATE TABLE IF NOT EXISTS investec_fraud_aml_gold.sherlock_sar_filings (
  sar_id        STRING,
  case_id       STRING,
...
DELETE FROM investec_fraud_aml_gold.sherlock_case_notes WHERE note_id LIKE 'NOTE-9%';
DELETE FROM investec_fraud_aml_gold.sherlock_case_actions WHERE action_id LIKE 'ACT-9%';
INSERT INTO investec_fraud_aml_gold.sherlock_case_notes
  (note_id, case_id, author, note, note_type, created_at)
VALUES
  ('NOTE-9001A','CASE-90001','Thandeka Nkosi','Alert opened: three cash deposits just under R25k over ~40h on a recently-opened Investec Private Bank account. Checking for linked accounts.','analyst', current_timestamp() - INTERVAL 3 HOURS),
...
INSERT INTO investec_fraud_aml_gold.sherlock_case_actions
