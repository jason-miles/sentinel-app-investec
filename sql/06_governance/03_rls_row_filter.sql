...
--   1. Investec app service principal (ec6d288f-…)   → full visibility (app keeps working)
...
-- query returns 200 but 0 rows. The client id below is the deployed investec-fraud-aml
-- app's service principal; re-point it if the app is recreated (get it from
-- `databricks apps get investec-fraud-aml`).
--
