# Investec — Fraud & AML Detection App on Databricks

A demo/PoC that answers, end-to-end: **"What kinds of alerts would indicate financial crime at Investec, and what do we do about them?"** — framed for a South African private banking bank (Investec Private Bank accounts, ZAR, FICA/FIC regulatory regime).

Built SQL-first (Lakeflow Declarative Pipelines) so a SQL-skilled team can own it. Python is confined to the app shell and a few AI/graph helpers. Competitive context for the Investec account: a best-of-breed data + AI layer on their existing **AWS** estate (coexistence, not rip-and-replace) — see `docs/`.
...
- **A — Hidden mule network:** a "boring" sub-threshold cash-structuring alert on one Investec Private Bank account expands, via entity resolution (shared device / IP / address, opened within a 3-week window), into a **7-account money-mule network** forwarding ~90% within 48h to one aggregator that remits cross-border SWIFT — with sibling accounts previously closed as false positives.
