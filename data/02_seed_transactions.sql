-- Investec — Fraud & AML SAMPLE DATA · Synthetic seeder (2/4): bulk ledger + card
-- transactions.  *** INVESTEC BANK DEMO DATA — 100% SYNTHETIC ***
-- Target ~2-3M ledger transactions over the trailing 12 months (PRD §10).
-- Investec context: retail channels skew to the Investec Private Bank banking app, card, ATM cash
...
SELECT
  ...
  element_at(array('Card purchase','Salary credit','SASSA grant','Investec Cash Send','Investec Private Bank transfer','Prepaid airtime & data','DebiCheck debit order','ATM cash withdrawal','Immediate payment (RTC)'), cast(pmod(txn_seq,9)+1 AS INT)) AS description,
  concat('INVAPP-', lpad(cast(pmod(acct_num * 40009, 90000000) AS BIGINT), 8, '0')) AS device_id,  -- client's own Investec app device
