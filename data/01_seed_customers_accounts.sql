-- Investec — Fraud & AML SAMPLE DATA · Synthetic seeder (1/4): customers, accounts,
-- third parties.  *** INVESTEC BANK DEMO DATA — 100% SYNTHETIC, NO REAL CLIENTS ***
...
-- Investec context: SA private mass-market SA retail bank specialist bank (high-net-worth). Signature product is the Investec Private Bank
-- account (transactional + savings pockets); SA 13-digit ID numbers, ZAR, a
-- township/metro footprint, and Investec's AWS-native source systems (enterprise data
...
SELECT
  ...
  -- SA 13-digit ID number (YYMMDD + sequence) — the format Investec captures at FICA
  ...
  -- Synthetic demo mailbox on a Investec sample domain (clearly not production)
  concat('client', id, '@demo.investec.co.za')                   AS email,
  ...
  -- Investec Investec Private Banking product tiers (private banking), weighted toward entry/active
  element_at(array('private_bank','private_client','private_client','wealth','business'), cast(pmod(id, 5) + 1 AS INT)) AS segment,
  ...
  -- Investec app device fingerprint (the channel most Investec clients use)
  concat('INVAPP-', lpad(cast(pmod(id * 40009, 90000000) AS BIGINT), 8, '0')) AS device_id,
  cast(date_add('2018-01-01', cast(pmod(id * 17, 2900) AS INT)) AS TIMESTAMP) AS onboarded_at,
  -- Investec channels: the Investec app, a branch, or a WhatsApp/USSD-assisted agent
  element_at(array('investec_app','branch','agent'), cast(pmod(id, 3) + 1 AS INT)) AS onboarding_channel,
  -- Investec's AWS-native estate (per their published stack): enterprise data lake,
  -- core banking, and the CRM feed — the systems this medallion conforms.
  element_at(array('investec_edl','core_banking','crm_360'), cast(pmod(id, 3) + 1 AS INT)) AS source_system,
...
SELECT
  ...
  -- Investec Private Bank account structure: transactional account, savings pockets, fixed-term savings, card
  element_at(array('private_bank_account','savings_pocket','fixed_savings','card'), cast(pmod(cust_num + acct_seq, 4) + 1 AS INT)) AS account_type,
  'ZAR'                                                              AS currency,
  ...
  element_at(array('investec_edl','core_banking','crm_360'), cast(pmod(cust_num, 3) + 1 AS INT)) AS source_system,
  current_timestamp()                                                AS _ingested_at
FROM expanded;
