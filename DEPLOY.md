# Investec Sentinel — Deployment Guide + Director's Notes

This is the Investec build of the Sentinel Fraud & AML app (alongside the Capitec and Nedbank
...
## 0. What makes this build "Investec"

| Dimension            | Value |
|----------------------|-------|
| Brand                | Investec deep blue `#003b5c` + signature red `#e2001a` (light + dark themes, logo, favicon) |
| Market framing       | Mass-market retail bank; **Investec Private Bank** account tiers; ZAR; SA metro + township footprint |
| Regulatory framing   | **FICA / FIC** (STRs & CTRs to the Financial Intelligence Centre via **goAML**), **SARB** Prudential Authority, **POPIA**, **NCA** |
| Filing institution   | `Investec Bank Limited` (goAML reporting entity `INVESTEC-ZA`) |
| Bundle name          | `investec_sentinel` |
| Nested pipeline bundle | `investec_fraud_aml_pipeline` |
| App name             | `investec-fraud-aml` |
| Schemas              | `investec_fraud_aml_bronze` / `_silver` / `_gold` in `elexon_app_for_settlement_acc_catalog` |
| Pipeline / jobs      | `investec_fraud_aml_pipeline_etl`, `investec_fraud_aml_daily_report`, `investec_fraud_aml_stream_trigger`, `investec_fraud_ml_retrain` |
...
### New capability surfaces added for the Investec brief
...
- RLS: `03_rls_row_filter.sql` sets a UC row filter on `sherlock_cases`. The app runs as
  ...
  returns 0 rows. Update the id from `databricks apps get investec-fraud-aml`.
...
# The pipeline RESOURCE KEY is `fraud_aml_pipeline_etl` (the investec_ prefix is only the
# display name). Streaming lanes need one file each first, or schema inference fails:
databricks fs mkdir dbfs:/Volumes/elexon_app_for_settlement_acc_catalog/investec_fraud_aml_bronze/landing/transactions
...
```
SENTINEL_GENIE_SPACE   = 01f194ad316e127191fec45fdd5fb6bc   # "Investec Fraud & AML Analyst"
SENTINEL_DASHBOARD_ID  = 01f194ad41b618fa8342f8a851b45507   # "Investec Sentinel — Executive Overview"
```
...
databricks genie create-space <warehouse_id> "$(cat serialized_space.json)" \
  --title "Investec Fraud & AML Analyst" --description "$(cat description.txt)"
# Dashboard:
databricks lakeview create --display-name "Investec Sentinel — Executive Overview" \
...
```
The committed `webroot/` is already built with Investec branding.
...
- App landing page shows the Investec blue/red hero; favicon is the blue tile + red "C".
